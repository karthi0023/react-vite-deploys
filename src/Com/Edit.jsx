...
export const Edit = ({ cart, head }) => {
  ...
  const [edit_row,setedit_row]=useState();
  const indexs=(index)=>{
         setedit_row(index);
  }
  const chagerate=(e)=>{
    if(e.key==="Enter"){ 
              const updatadat=[...cart];
              updatadat[edit_row][5]=e.target.value;
              
              localStorage.setItem("rows",JSON.stringify(updatadat));
      ...
      <div className='body-overall'>
        <div className='bill-box'>
          ...
          <div className='item-box'>
            ...
            <input type="text" placeholder='Amonut' value={rate} name="" id="" />
            <input type="text" placeholder='QTV' onChange={(e) => qtydata(e)} name="" id="" />
            ...
            <p onClick={() => additem()} className='add-item'>Add</p>
            </form>
          </div>
            ...
            <tbody>
              {
                sale.map((maps) => (
                  <tr key={maps.id}>
                    ...
                    <input type="text" className='edit-input' onKeyDown={(e)=>chagerate(e)} />
                  </tr>
                ))
              }
            </tbody>
            ...
            <tbody>
              {cart.filter((item) => {
                ...
                <tr className='search-tr td' onClick={() => handles(cl)} key={i}>
                  {cl.map((c, l) => (
                    <td className='search-tr' onClick={()=>indexs(i,l)} key={l}>{c}</td>
